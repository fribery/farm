...
export default function HangarScreen({ user, updateGameData, availableSlots }) {
  ...
  return (
    <div className="hangar-section">
      {/* Доступные корабли для запуска */}
      {user.game_data?.availableShips?.length > 0 && (
        <div className="available-ships-container">
          <h3 className="section-title">
            <span className="title-icon">🚀</span>
            Готовы к запуску ({ships.filter(s => s.status === 'on_mission').length}/{availableSlots})
          </h3>
          
          {ships.filter(s => s.status === 'on_mission').length >= availableSlots && (
            <div className="slots-full-message">
              <span className="warning-icon">⚠️</span>
              <span>Все слоты ангара заняты! Освободите место или расширьте ангар на верфи.</span>
            </div>
          )}
          
          <div className="ships-row">
            {user.game_data.availableShips.map((shipId, index) => {
              const shipConfig = getShipInfo(shipId)
              if (!shipConfig) return null
              
              const canLaunch = ships.filter(s => s.status === 'on_mission').length < availableSlots
              const alreadyOwned = ships.some(s => s.shipId === shipId && s.status === 'docked')
              
              return (
                <div 
                  key={index} 
                  className={`ship-card-inline ${!canLaunch ? 'disabled' : ''}`}
                  title={!canLaunch ? 'Нет свободных слотов' : `${alreadyOwned ? 'Запустить' : 'Приобрести'} ${shipConfig.name}`}
                >
                  <div className="ship-inline-top">
                    <div className="ship-inline-emoji">
                      {shipConfig.emoji}
                    </div>
                    {alreadyOwned && (
                      <div className="ship-owned-badge">
                        ✅
                      </div>
                    )}
                  </div>
                  
                  <div className="ship-inline-info">
                    <div className="ship-inline-name">
                      {shipConfig.name.replace(shipConfig.emoji, '').trim()}
                    </div>
                    <div className="ship-inline-details">
                      <div className="ship-detail">
                        <span className="detail-icon">⏱️</span>
                        <span>{formatTime(shipConfig.missionDuration)}</span>
                      </div>
                      <div className="ship-detail">
                        <span className="detail-icon">💰</span>
                        <span>+{shipConfig.baseIncome}кр</span>
                      </div>
                      <div className="ship-detail">
                        <span className="detail-icon">🛡️</span>
                        <span>-{shipConfig.durability.decayPerMission}%</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      if (canLaunch) {
                        if (alreadyOwned) {
                          launchMission(shipConfig.id, shipConfig.name)
                        } else {
                          window.showInfo('Сначала приобретите корабль на верфи!')
                        }
                      }
                    }}
                    disabled={!canLaunch}
                    className={`launch-btn-inline ${canLaunch ? '' : 'disabled'}`}
                  >
                    {alreadyOwned ? 'Запустить' : 'Купить'}
                  </button>
                </div>
              )
            })}
