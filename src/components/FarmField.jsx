...
export default function FarmField({ user, updateGameData, availableSlots }) {
  ...
  const harvestField = (fieldId) => {
    ...
    const checkLevelUp = (gameData) => {
      ...
      const nextLevelExp = [0, 100, 250, 500, 1000, 2000, 3500, 5000, 7500, 10000, 15000, 22500, 35000];
      const neededExp = nextLevelExp[level] || 2000;
        ...
        {fields.length === 0 ? (  
          ...
          <div className="fields-grid-compact">
            {fields.map(field => {
              ...
              const plantEmoji = getPlantEmoji(field.plantId)
                ...
                >
                  ...
                  <div className="field-header-compact">
                    <div className="field-info-compact">
                      {/* Иконка теперь в названии растения */}
                      <h4 
                        className="field-name"
                        data-emoji={plantEmoji}
                      >
                        {plant?.name || field.name}
                      </h4>
